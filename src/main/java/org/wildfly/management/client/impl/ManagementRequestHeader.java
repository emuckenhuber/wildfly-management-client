...
class ManagementRequestHeader extends ManagementProtocolHeader {
    ...
    public void read(final DataInput input) throws IOException {
        StreamUtils.expectHeader(input, ManagementProtocol.REQUEST_ID);
        requestId = input.readInt();
        StreamUtils.expectHeader(input, ManagementProtocol.BATCH_ID);
        batchId = input.readInt();
        StreamUtils.expectHeader(input, ManagementProtocol.OPERATION_ID);
        operationId = input.readByte();
        StreamUtils.expectHeader(input, ManagementProtocol.ONE_WAY);
        oneWay = input.readBoolean();
        StreamUtils.expectHeader(input, ManagementProtocol.REQUEST_BODY);
    }
